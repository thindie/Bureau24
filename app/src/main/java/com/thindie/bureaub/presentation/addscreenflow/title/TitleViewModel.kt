...
internal class TitleViewModel(
    ...
    fun onAction(action: Action) {
        viewModelScope.launch {
            when (action) {
                is Action.OnChangeField -> title = action.value
                is Action.OnComplete -> repository.addNote(title, "", tag = null)
            }
        }
    ...
    override fun onUserFlowEvent(t: AddFlowEvent) {
        viewModelScope.launch {
            when (t) {
                AddFlowEvent.Desc -> routeFlow.emit(Destination.AddDescription)
